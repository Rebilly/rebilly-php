...
- Added `TrialOnlyPlanDetails` on `TrialOnlyPlanDetails` (response only) **(breaking)**
...
- Removed schema `KhelocardCard`
- Removed schema `KhelocardCardToken`
- Removed schema `PayoutRequestSplit`
- Removed `RebillyShipping` on `RebillyShipping, StorefrontChangeQuote, StorefrontReactivationQuote`
- Removed schema `StorefrontKhelocardCard`
- Removed `StorefrontKhelocardCard` on `StorefrontKhelocardCard` (response only)
...
- Made nullable `quantityFilled` on `OneTimeSale, Subscription` **(breaking)**
...
- Discriminator mapping added `IMerchant, PayGlocal`
- Discriminator mapping changed `Khelocard` on `AlternativeInstrument, AlternativePaymentToken`
...
- `not` no longer accepts: `Khelocard`
...
- api-path-removed-without-deprecation (3x) — e.g. api path removed without deprecation
...
- discriminator-added on `200` (response only)
- discriminator-added on `201` (response only)
- discriminator-added on `items/` (response only)
- Added discriminator mapping for `IMerchant, PayGlocal` (response only)
- Added discriminator mapping for `PayGlocal, IMerchant`
- Added discriminator mapping for `users`
- Changed discriminator mapping for `Khelocard` on `AlternativeInstrument, AlternativePaymentToken, OneTimeOrder, RecurringOrder`
...
- endpoint-deprecated (5x) — e.g. endpoint deprecated
...
- Added discriminator variant to `TrialOnlyPlan` on `TrialOnlyPlan` (request only)
- Added discriminator variant to `TrialOnlyPlanDetails` on `TrialOnlyPlanDetails` (response only)
...
- Removed discriminator variant from `StorefrontKhelocardCard` on `StorefrontKhelocardCard` (response only)
...
- parameter-max-decreased on `query` (request only)
...
- type-specialized on `type` (response only)
...
- webhook-removed on `orders-experimental-subscription-renewal-reminder` (response only)
