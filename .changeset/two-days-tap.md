...
- Added optional response property `quantityFilled` on `QuoteSubscriptionItem` — `GET /quotes`, `POST /quotes` + 13 more
- Added optional request property `quantityFilled` on `QuoteSubscriptionItem` — `POST /quotes`, `PUT /quotes/{id}` + 8 more
